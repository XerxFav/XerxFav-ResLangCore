...
# === CLI helper ===
def describe_protoform() -> str:
    """
    Автоматически собирает список методов класса ProtoFormOperator
    и возвращает описание оператора.
    """
    methods = [
        name for name, obj in inspect.getmembers(ProtoFormOperator, inspect.isfunction)
        if not name.startswith("__")
    ]

    description = "Protoform Operator — оператор для работы с абстрактными фазовыми структурами.\n"
    description += "Методы:\n"
    for m in methods:
        description += f"  - {m}\n"
